...
With Segment, you can easily manage data and integrations with services across your Growth, Product, and Marketing stack. By tracking events and users via Segment's API and libraries, you can send your product's data to all of your analytics/marketing platforms, with minimal instrumentation code. They offer support for most platforms, including iOS, Android, JavaScript, Node.js, PHP, and more.

This guide is meant to complement [Segment's integration documentation](https://segment.com/docs/integrations/amplitude/). It provides additional details on the different types of Segment integrations you can use, how they affect your data in Amplitude, and instructions for setting up the integrations.
...
6. [Build a chart](https://help.amplitude.com/hc/en-us/articles/230290208-Event-Segmentation) in Amplitude and start exploring your data.
...
- By enabling Track Session Events, you can ensure the SDKs will automatically send` ![amplitude_logo.png](https://help.amplitude.com/hc/article_attachments/4416657571995/amplitude_logo.png) Start Session `and `![amplitude_logo.png](https://help.amplitude.com/hc/article_attachments/4416657571995/amplitude_logo.png) End Session `events to mark the start and end of a user's mobile session.
- When Amplitude's SDKs are installed, you can directly interact with them. See our articles on [iOS SDK Installation](https://developers.amplitude.com/docs/ios-setup) and [Android SDK installation](https://developers.amplitude.com/docs/android-setup) to learn more.
...
Without session tracking, events have a session ID of -1, so events triggered by a user during the same session will **not** be grouped together when viewing that [user's timeline](https://help.amplitude.com/hc/en-us/articles/229313067-Amplitude-2-0-User-Activity-Tab#individual-event-streams) in Amplitude. [Session length calculations](https://help.amplitude.com/hc/en-us/articles/115002323627#how-sessions-are-tracked) will not be available in Amplitude, so the [User Sessions](https://help.amplitude.com/hc/en-us/articles/231275508-User-Sessions) chart will not display any data. Amplitude's [Pathfinder](https://help.amplitude.com/hc/en-us/articles/229792148-Pathfinder) and [Pathfinder Users](https://help.amplitude.com/hc/en-us/articles/235777567-Pathfinder-Users) charts will also not be able to display out-of-session events alongside events within a session. You will also not be able to perform [session-based Funnel Analysis](https://help.amplitude.com/hc/en-us/articles/229951267#session-based-funnels) effectively.

Without Amplitude's SDKs, you have to map them manually to track user properties such as device type and operating system into Amplitude, as described [here](https://segment.com/docs/integrations/amplitude/#special-properties).
...
!!!tip
    See the [JavaScript SDK documentation](/../data/sdks/javascript) for additional capabilities provided directly by our JavaScript SDK.

There are additional settings for the JavaScript integration you can configure (in the Advanced Settings of your Segment Amplitude integration panel), without needing to change your instrumentation:
...
- Register the Amplitude integration with Segment's SDK. When building the Analytics object, append `.use(AmplitudeIntegration.FACTORY)` before the `.build()`. It would look something like this:
    ```java
...
## Mappings between Segment's API and Amplitude's API

Segment and Amplitude use slightly different terms to describe many of the same concepts. The following table shows the mapping between the two APIs:
...
### Import Segment data into Amplitude Recommend
